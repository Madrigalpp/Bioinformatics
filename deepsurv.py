...
class DeepSurv:
    ...
    standardize = False,
        ...
        """
        n_in：输入节点的数量 - 这是输入层的大小，表示特征或输入变量的数量。

        learning_rate：学习率 - 这个参数控制了优化过程中的步长。它决定了模型在训练的每个迭代中更新权重的程度。
        
        lr_decay：Power 学习率衰减的系数 - 这是一个应用于学习率的因子，用于随时间衰减学习率，帮助模型更有效地收敛。
        
        L2_reg：L2 权重衰减正则化的系数 - L2 正则化有助于防止模型过拟合。
        """
