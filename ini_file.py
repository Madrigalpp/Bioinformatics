...
def create_ini_file(file_name, output_ini_path, first_dimension):
    ...
    config.set('network', 'dims', f'[{first_dimension}, 128, 64, 64, 1]')
    config.set('network', 'activation', '"ReLU"')
    config.set('network', 'l2_reg', '0')

    with open(output_ini_path, 'w') as configfile:
        config.write(configfile)

your_directory_path = 'K_fold5_data'
...
#         e = dataset.get('x')
#
#         file_data_dict[file_name] = e[1].shape
#
...
#
# for file_name, dimensions in file_dimensions.items():
#     ini_file_path = os.path.join(configs_folder, os.path.splitext(file_name)[0] + '.ini')
#     create_ini_file(file_name, ini_file_path, dimensions[0])
#
#
#
...
for subdir in subdirectories:
    ...
    for file_path in files_in_subdir:
        ...
        file_data_dict[file_path] = e[1].shape  # Change filename to filepath
...
for file_path, dimensions in file_dimensions.items():  # Change filename to filepath
